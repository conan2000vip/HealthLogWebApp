...
public class StepService {
	...
	public Map<String, Object> list(Long profileId, Long currentUserId, LocalDate from, LocalDate to, int page) {
		...
		// ---- Latest record / 最新記録 ----
		Optional<Step> latestOpt = stepRepository.findTopByProfile_IdOrderByRecordedDateDescIdDesc(profileId);
		...
		Integer goalRatePercent = (todaySteps != null && goal != null && goal > 0)
				? (int) Math.round(todaySteps * 100.0 / goal) : null;
	...
	private BigDecimal calculateGoalRate(Integer steps, Integer goal) {
		...
		return BigDecimal.valueOf(steps).multiply(BigDecimal.valueOf(100))
	...
	// create()
	// ---------------------------------------------------------
	public Step create(Long profileId, Long currentUserId, Step step) {
		Profile profile = findProfile(profileId, currentUserId);
		validateStepInput(step);

		Optional<Step> existing = stepRepository.findFirstByProfile_IdAndRecordedDate(profileId, step.getRecordedDate());
		if (existing.isPresent()) {
			throw new BusinessException(HttpStatus.BAD_REQUEST, "この日にはすでに歩数データが登録されています");
		}

		step.setProfile(profile);
		if (step.getMemo() != null) {
			step.setMemo(step.getMemo().trim());
		}
	...
	public Step update(Long profileId, Long currentUserId, Long logId, Step input) {
		...
		Optional<Step> sameDate = stepRepository.findFirstByProfile_IdAndRecordedDate(profileId, input.getRecordedDate());
	...
	private List<Step> fetchLogs(Long profileId, LocalDate from, LocalDate to) {
		...
		if (from != null) {
			return stepRepository.findByProfile_IdAndRecordedDateGreaterThanEqualOrderByRecordedDateDesc(profileId, from);
		}
		if (to != null) {
	...
	private Page<Step> fetchLogsPage(Long profileId, LocalDate from, LocalDate to, Pageable pageable) {
		if (from != null && to != null) {
			return stepRepository.findByProfile_IdAndRecordedDateBetweenOrderByRecordedDateDesc(profileId, from, to, pageable);
		}
		if (from != null) {
			return stepRepository.findByProfile_IdAndRecordedDateGreaterThanEqualOrderByRecordedDateDesc(profileId, from, pageable);
		}
		if (to != null) {
			return stepRepository.findByProfile_IdAndRecordedDateLessThanEqualOrderByRecordedDateDesc(profileId, to, pageable);
		}
	...
	private ChartData buildChartData(List<Step> logs, String chartMode, LocalDate from, LocalDate to) {
		...
		if ("DAY".equals(chartMode)) {
			Map<LocalDate, Integer> daily = logs.stream().filter(step -> step.getRecordedDate() != null)
					.collect(Collectors.toMap(Step::getRecordedDate, step -> step.getSteps() == null ? 0 : step.getSteps(),
			...
			Map<String, List<Step>> monthly = logs.stream().collect(Collectors.groupingBy(
					step -> step.getRecordedDate().getYear() + "-" + String.format("%02d", step.getRecordedDate().getMonthValue()),
			...
			Map<Integer, List<Step>> yearly = logs.stream().collect(Collectors.groupingBy(
					step -> step.getRecordedDate().getYear(), TreeMap::new, Collectors.toList()));
